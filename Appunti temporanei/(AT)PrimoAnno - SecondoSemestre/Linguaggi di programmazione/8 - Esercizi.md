...
Caso 4) Creiamo diversi casi
	...
	Caso 4.3:Caso 4.3: $v \neq \lambda \ x \neq \lambda$; $v$ contiene solo delle a e x contiene solo delle $b$.  In questo caso quindi avremmo un numero di $b$ pari al numero $a$: $p+1\leq \#(b) = \#(a) \leq p + p -1$ (p-1 poiché almeno una b è contenuta nelle $w$). Quindi $uv^2wx^2y \not\in L \text{ poichè } \#(a) \wedge \#(b) \neq \#(c)$ 
	(Qui abbiamo pompato per )
